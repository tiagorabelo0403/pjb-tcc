...
import com.tcc.pjb.backend.ai.financeira.policy.FinancialSafetyGate;
...
@Component
public class FinanceiraRelatorPhase implements CognitivePhase {

    private final FinanceiraAiVersionSelector selector;

    public FinanceiraRelatorPhase(FinanceiraAiVersionSelector selector) {
    ...
    public void execute(AgentExecutionContext ctx) {
        ...
        IAResponse base = selector.process(req, ctx.version(), ctx.capability());

        String texto = FinancialSafetyGate.apply(base != null ? base.getTexto() : "");
        ...
        if (needUncertainty) {
            ...
            sb.append("\n\n[Incertezas e Próximos Dados Necessários]\n")
              .append("- Evite decisões automáticas: valide com documentos e parâmetros do caso.\n");
            ...
            if (conflictRisk) {
                sb.append("- Ambiguidade entre fontes: refine período/UF/base regulatória e amplie as evidências.\n");
            }
            if ((residual != null && residual >= 0.55) || (incons != null && incons >= 0.55)) {
                if (resolved) {
                    sb.append("- Houve conflito, mitigado por fonte âncora (maior base regulatória/autoridade); ainda requer validação.\n");
                } else {
                    sb.append("- Conflito/inconsistência relevante: delimite recorte temporal e base normativa.\n");
                }
            }
            texto = sb.toString();
        }
        ...
        pipelineMeta.put("financial_ai_runtime_descriptor", selector.descriptor(ctx.version()));
