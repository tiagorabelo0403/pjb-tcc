...
@Slf4j
@Service
public class FinanceiraCognitivePipelineOrchestrator {

    private final FinanceiraStrategistPhase strategist;
    private final FinanceiraResearcherPhase researcher;
    private final FinanceiraRelatorPhase relator;
    private final AiMicrometerTelemetry telemetry;
    private final Clock pjbClock;

    private final Executor aiTelemetryExecutor;

    public FinanceiraCognitivePipelineOrchestrator(
            FinanceiraStrategistPhase strategist,
            FinanceiraResearcherPhase researcher,
            FinanceiraRelatorPhase relator,
    ...
    private IAResponse failFastResponse(AgentExecutionContext ctx,
        ...
        IAResponse out = IAResponse.builder()
                .origem("FINANCEIRA_PIPELINE_" + ctx.version().name())
    ...
    private void recordTelemetryAsync(AgentExecutionContext ctx,
        ...
        aiTelemetryExecutor.execute(() -> {
            try {
                if (think != null) telemetry.recordPipelinePhase(AiTelemetryDomain.FINANCE, ctx.capability(), ctx.version(), outcome, CognitivePhaseName.THINK.name(), think);
                if (rag != null) telemetry.recordPipelinePhase(AiTelemetryDomain.FINANCE, ctx.capability(), ctx.version(), outcome, CognitivePhaseName.RAG.name(), rag);
                if (respond != null) telemetry.recordPipelinePhase(AiTelemetryDomain.FINANCE, ctx.capability(), ctx.version(), outcome, CognitivePhaseName.RESPOND.name(), respond);
                if (total != null) telemetry.recordPipelinePhase(AiTelemetryDomain.FINANCE, ctx.capability(), ctx.version(), outcome, CognitivePhaseName.TOTAL.name(), total);
                if (failFast) telemetry.recordPipelineFailFast(AiTelemetryDomain.FINANCE, ctx.capability(), ctx.version(), outcome);
                ...
                if (qObj instanceof Map<?, ?> qm) {
                    ...
                    telemetry.recordEvidenceQuality(AiTelemetryDomain.FINANCE, ctx.capability(), ctx.version(), outcome, score, conflict);
                }
                ...
                if (cObj instanceof Map<?, ?> cm) {
                    ...
                    telemetry.recordEvidenceSignals(AiTelemetryDomain.FINANCE, ctx.capability(), ctx.version(), outcome, contradiction, inconsistency);
                }
                ...
                if (rObj instanceof Map<?, ?> rm) {
                    ...
                    telemetry.recordContradictionResolution(AiTelemetryDomain.FINANCE, ctx.capability(), ctx.version(), outcome, resolved);
                }
                ...
                if (pObj instanceof Map<?, ?> pm) {
                    ...
                    telemetry.recordMinQuestions(AiTelemetryDomain.FINANCE, ctx.capability(), ctx.version(), outcome, count);
                }

            } catch (Throwable t) {
                log.warn("[AI][FINANCEIRA][PIPELINE] telemetry error traceId={} err={}", ctx.traceId(), t.toString());
            }
        });
    }
}
