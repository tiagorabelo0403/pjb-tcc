...
public final class RecursalPanelHandoffTrackFactory {

    private RecursalPanelHandoffTrackFactory() {
    }
    ...
    public static RecursalAutomationWorkspaceTrackView buildTrack(String recursoPrincipal,
        ...
        return new RecursalAutomationWorkspaceTrackView(
                "PAINEL_JURISDICIONAL_COMPETENTE",
                "Painel jurisdicional competente",
                recursoPrincipal,
                RecursalJurisdictionPanelBlueprint.secoes(recursoPrincipal, request),
                RecursalJurisdictionPanelBlueprint.checklist(recursoPrincipal, request).entrySet().stream()
                        .map(entry -> new RecursalAutomationWorkspaceChecklistItemView(entry.getKey(), entry.getValue(), true))
                        .toList(),
                List.copyOf(RecursalJurisdictionPanelBlueprint.alertas(recursoPrincipal, request))
        );
    }
}
