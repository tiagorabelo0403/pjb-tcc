...
public class DelegationTokenService {
    ...
    public static final String PREFIX = "PJB-DCPv1";

    private final ObjectMapper objectMapper;
    private final KeyMaterialService keyMaterialService;

    public DelegationTokenService(ObjectMapper objectMapper,
                                 KeyMaterialService keyMaterialService) {
        this.objectMapper = objectMapper;
        this.keyMaterialService = keyMaterialService;
    }

    public String sign(DelegationTokenPayload payload) {
        try {
            byte[] json = objectMapper.writeValueAsBytes(payload);
            String p64 = base64Url(json);
            String signingInput = PREFIX + "." + p64;
            byte[] sig = hmac(signingInput.getBytes(StandardCharsets.UTF_8), keyMaterialService.getDelegationSigningKey());
            return signingInput + "." + base64Url(sig);
        } catch (Exception e) {
            log.error("CRITICAL: falha ao assinar DCP", e);
            throw new IllegalStateException("Falha crítica ao assinar token DCP", e);
        }
    }

    public DelegationTokenPayload verifyAndDecode(String token) {
        if (token == null || token.isBlank()) {
            throw new IllegalArgumentException("token DCP ausente");
        }
        ...
        byte[] sigExpected = hmac(signingInput.getBytes(StandardCharsets.UTF_8), keyMaterialService.getDelegationSigningKey());
        ...
        try {
            ...
            DelegationTokenPayload payload = objectMapper.readValue(payloadJson, DelegationTokenPayload.class);
