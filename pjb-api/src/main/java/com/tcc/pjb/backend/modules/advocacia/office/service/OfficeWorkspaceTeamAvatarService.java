...
public class OfficeWorkspaceTeamAvatarService {
    ...
    public ObjectReadResult read(Long userId, HttpServletRequest request) throws IOException {
        var current = currentUserService.getRequired();
        if (Objects.equals(current.getId(), userId)) {
            return userAvatarService.read(userId);
        }
        ...
        if (!allowed || usuarioAvatarRepository.findByUsuarioId(userId).isEmpty()) {
