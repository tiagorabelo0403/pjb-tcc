...
public final class RecursalAttorneyDashboardTrackFactory {

    private RecursalAttorneyDashboardTrackFactory() {
    }
    ...
    public static RecursalAutomationWorkspaceTrackView buildTrack(String recursoPrincipal,
        ...
        return new RecursalAutomationWorkspaceTrackView(
                ...
                "Painel recursal completo do advogado",
                recursoPrincipal,
                RecursalAttorneyDashboardBlueprint.secoes(recursoPrincipal, request),
                RecursalAttorneyDashboardBlueprint.checklist(recursoPrincipal, request).entrySet().stream()
                        .map(entry -> new RecursalAutomationWorkspaceChecklistItemView(entry.getKey(), entry.getValue(), true))
                        .toList(),
                List.copyOf(RecursalAttorneyDashboardBlueprint.alertas(recursoPrincipal, request))
        );
    }
}
