...
public final class RecursalSecretariatParityTrackFactory {

    private RecursalSecretariatParityTrackFactory() {
    }
    ...
    public static RecursalAutomationWorkspaceTrackView buildTrack(String recursoPrincipal,
        ...
        return new RecursalAutomationWorkspaceTrackView(
                ...
                "Secretaria multigrau reforçada",
                recursoPrincipal,
                RecursalSecretariatParityBlueprint.secoes(recursoPrincipal, request),
                RecursalSecretariatParityBlueprint.checklist(recursoPrincipal, request).entrySet().stream()
                        .map(entry -> new RecursalAutomationWorkspaceChecklistItemView(entry.getKey(), entry.getValue(), true))
                        .toList(),
                List.copyOf(RecursalSecretariatParityBlueprint.alertas(recursoPrincipal, request))
        );
    }
}
