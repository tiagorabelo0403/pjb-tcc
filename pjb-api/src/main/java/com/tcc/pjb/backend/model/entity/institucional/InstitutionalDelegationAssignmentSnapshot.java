...
@Entity
@Table(name = "tb_inst_delegation_assignment_snapshot",
        indexes = {
                @Index(name = "idx_inst_delegation_exp", columnList = "expedicao_uuid, updated_at"),
                @Index(name = "idx_inst_delegation_proc", columnList = "processo_id, updated_at"),
                @Index(name = "idx_inst_delegation_status", columnList = "status_codigo, updated_at")
        },
        uniqueConstraints = @UniqueConstraint(name = "uk_inst_delegation_assignment_id", columnNames = "assignment_id"))
public class InstitutionalDelegationAssignmentSnapshot {
    ...
    @Column(name = "assignment_id", nullable = false, length = 160)
    ...
    @Column(name = "tipo_fluxo", nullable = false, length = 80)
    private String tipoFluxo;

    @Column(name = "delegado_usuario_id", nullable = false)
    private Long delegadoUsuarioId;
    ...
    protected InstitutionalDelegationAssignmentSnapshot() {
    }

    public InstitutionalDelegationAssignmentSnapshot(String assignmentId,
                                                    ...
                                                    Instant createdAt,
                                                    Instant updatedAt) {
        this.assignmentId = require(assignmentId, "assignmentId");
        ...
        this.tipoFluxo = require(tipoFluxo, "tipoFluxo");
        this.delegadoUsuarioId = delegadoUsuarioId;
