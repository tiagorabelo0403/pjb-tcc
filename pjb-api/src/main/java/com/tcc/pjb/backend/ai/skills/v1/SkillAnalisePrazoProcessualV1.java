...
public class SkillAnalisePrazoProcessualV1 implements IASkill {
    ...
    public boolean suporta(IARequest request) {
        ...
        return acao.contains("ANALISE_CONTEXTO");
    }
    ...
    public IAResponse executar(IARequest request, Map<String, Object> context) {
        ...
        IAResponse response = IAResponse.builder()
                .origem(getNome())
        ...
        response.getMetadados().put("intencao", intencao.name());

        if (numeroProcesso != null) {
            response.getMetadados().put("numero_processo", numeroProcesso);
        }
