...
public final class RecursalActorContextExperienceTrackFactory {

    private RecursalActorContextExperienceTrackFactory() {
    }
    ...
    public static RecursalAutomationWorkspaceTrackView buildTrack(String recursoPrincipal,
        ...
        return new RecursalAutomationWorkspaceTrackView(
                ...
                "Shell contextual por ator e perfil",
                recursoPrincipal,
                RecursalActorContextExperienceBlueprint.secoes(recursoPrincipal, request),
                RecursalActorContextExperienceBlueprint.checklist(recursoPrincipal, request).entrySet().stream()
                        .map(entry -> new RecursalAutomationWorkspaceChecklistItemView(entry.getKey(), entry.getValue(), true))
                        .toList(),
                List.copyOf(RecursalActorContextExperienceBlueprint.alertas(recursoPrincipal, request))
        );
    }
}
