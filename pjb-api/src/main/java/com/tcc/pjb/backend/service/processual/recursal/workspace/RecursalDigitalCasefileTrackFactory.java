...
public final class RecursalDigitalCasefileTrackFactory {

    private RecursalDigitalCasefileTrackFactory() {
    }
    ...
    public static RecursalAutomationWorkspaceTrackView buildTrack(String recursoPrincipal,
        ...
        return new RecursalAutomationWorkspaceTrackView(
                ...
                "Autos digitais recursais detalhados",
                recursoPrincipal,
                RecursalDigitalCasefileBlueprint.secoes(recursoPrincipal, request),
                RecursalDigitalCasefileBlueprint.checklist(recursoPrincipal, request).entrySet().stream()
                        .map(entry -> new RecursalAutomationWorkspaceChecklistItemView(entry.getKey(), entry.getValue(), true))
                        .toList(),
                List.copyOf(RecursalDigitalCasefileBlueprint.alertas(recursoPrincipal, request))
        );
    }
}
