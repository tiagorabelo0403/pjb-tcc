...
@Service
public class CidadaoJulgamentoService {
  ...
  public CidadaoJulgamentoService(JulgamentoColegiadoService julgamentoService,
  ...
  public JulgamentoResumoDto detalheJulgamento(Long julgamentoId) {
    ...
    if (p == null) {
      throw new org.springframework.web.server.ResponseStatusException(
          org.springframework.http.HttpStatus.INTERNAL_SERVER_ERROR, "julgamento sem processo");
    }
    authz.requireReadVotosColegiados(p);
  ...
  private JulgamentoResumoDto toResumo(JulgamentoColegiado j, List<VotoColegiado> votos, boolean includeSse) {
    ...
    String sseUrl = includeSse ? ("/api/v1/cidadao/julgamentos/" + j.getId() + "/votos/stream") : null;
