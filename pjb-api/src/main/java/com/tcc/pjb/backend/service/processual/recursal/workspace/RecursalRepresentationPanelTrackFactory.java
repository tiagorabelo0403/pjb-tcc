...
public final class RecursalRepresentationPanelTrackFactory {

    private RecursalRepresentationPanelTrackFactory() {
    }
    ...
    public static RecursalAutomationWorkspaceTrackView buildTrack(String recursoPrincipal,
        ...
        return new RecursalAutomationWorkspaceTrackView(
                ...
                "Painel recursal de partes e representantes",
                recursoPrincipal,
                RecursalRepresentationPanelBlueprint.secoes(recursoPrincipal, request),
                RecursalRepresentationPanelBlueprint.checklist(recursoPrincipal, request).entrySet().stream()
                        .map(entry -> new RecursalAutomationWorkspaceChecklistItemView(entry.getKey(), entry.getValue(), true))
                        .toList(),
                List.copyOf(RecursalRepresentationPanelBlueprint.alertas(recursoPrincipal, request))
        );
    }
}
