...
public final class RecursalExternalCertificatesTrackFactory {

    private RecursalExternalCertificatesTrackFactory() {
    }
    ...
    public static RecursalAutomationWorkspaceTrackView buildTrack(String recursoPrincipal,
        ...
        return new RecursalAutomationWorkspaceTrackView(
                ...
                "Certidões externas recursais",
                recursoPrincipal,
                RecursalExternalCertificatesBlueprint.secoes(recursoPrincipal, request),
                RecursalExternalCertificatesBlueprint.checklist(recursoPrincipal, request).entrySet().stream()
                        .map(entry -> new RecursalAutomationWorkspaceChecklistItemView(entry.getKey(), entry.getValue(), true))
                        .toList(),
                List.copyOf(RecursalExternalCertificatesBlueprint.alertas(recursoPrincipal, request))
        );
    }
}
