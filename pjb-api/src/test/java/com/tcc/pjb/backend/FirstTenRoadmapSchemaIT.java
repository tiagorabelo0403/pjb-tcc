...
class FirstTenRoadmapSchemaIT extends PjbIntegrationTestBase {

    @Autowired
    private JdbcTemplate jdbcTemplate;

    @Test
    void roadmap_migrations_v178_a_v184_devem_ter_tabelas_colunas_indices_e_fks_criticos() {
        assertColumns("pjb_icp_certificate_cache", "issuer_dn", "serial_hex", "valid_until", "revoked");
        assertColumns("pjb_icp_signature_event", "processo_id", "doc_hash", "profile_candidate", "validation_ok");
        assertColumns("pjb_mni_remessa", "processo_id", "tribunal_destino", "status", "proximo_retry_em");
        assertColumns("pjb_mni_recepcao", "tribunal_origem", "numero_unificado", "mni_payload_hash", "status");
        assertColumns("pjb_datajud_feed_checkpoint", "tribunal_codigo", "last_processo_id", "total_sent", "updated_at");
        assertColumns("pjb_audiencia_custodia", "processo_id", "prazo_limite_24h", "magistrado_id", "resultado");
        assertColumns("pjb_medida_cautelar", "processo_id", "tipo", "ativa", "proximo_comparecimento");
        assertColumns("pjb_bnmp_consulta_log", "processo_id", "cpf_consultado", "mandado_ativo", "operador_id");
        assertColumns("pjb_sisbajud_operacao", "processo_id", "status", "operador_id", "authz_trail_id");
        assertColumns("pjb_renajud_restricao", "processo_id", "status", "operador_id", "authz_trail_id");
        ...
        assertColumns("pjb_deposito_recursal", "processo_id", "instancia", "valor_depositado", "status");
        assertIndex("pjb_icp_certificate_cache", "idx_icp_cert_valid");
        assertIndex("pjb_mni_remessa", "idx_mni_remessa_status");
        assertIndex("pjb_audiencia_custodia", "idx_custodia_status");
