...
class NationalRecursalStateMachineStateRefinementTest {
    ...
    @Test
    void deveMarcarSustentacaoRealizadaAntesDoDesfecho() {
        ...
        RecursalStateSnapshot snapshot = stateMachine.initialSnapshot("resp-sustentacao-realizada", context, species, routePlan);

        snapshot = flowToMerit(snapshot, context, species, routePlan);
        ...
        assertThat(stateMachine.availableEvents(snapshot, species, routePlan))
                .contains(RecursalTransitionEvent.PROVER, RecursalTransitionEvent.NEGAR_PROVIMENTO)
                .doesNotContain(RecursalTransitionEvent.PEDIR_PAUTA_SUSTENTACAO);
    }

    @Test
    void deveSepararAplicacaoDePrecedenteEDistinguishingAntesDaRetomada() {
        RecursalCaseContext context = upperCourtContext(false);
        RecursalSpecies species = new RecursoEspecial(true, true, false, false);
        RecursalRoutePlan routePlan = catalog.route(context, species);
        RecursalStateSnapshot snapshot = stateMachine.initialSnapshot("resp-precedente", context, species, routePlan);
        ...
        snapshot = move(snapshot, context, species, routePlan, RecursalTransitionEvent.APLICAR_PRECEDENTE);

        assertThat(snapshot.state()).isEqualTo(RecursalLifecycleState.PRECEDENTE_APLICADO);
    ...
    @Test
    void devePermitirFilaEspecificaDePrecatorioQuandoModalidadeJaVemResolvida() {
        RecursalCaseContext context = upperCourtContext(true);
        RecursalSpecies species = new RecursoEspecial(true, true, false, false);
        RecursalRoutePlan routePlan = catalog.route(context, species);
        RecursalStateSnapshot snapshot = stateMachine.initialSnapshot("resp-precatorio-especifico", context, species, routePlan);

        snapshot = flowToMerit(snapshot, context, species, routePlan);
        snapshot = stateMachine.transition(new RecursalTransitionCommand(
                snapshot,
                context,
                species,
                RecursalTransitionEvent.PROVER,
                "tester",
                Instant.parse("2026-03-11T18:00:00Z"),
                new RecursalTransitionDetails(null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, "REQ-1", "PRECATORIO")
        ), routePlan);

        assertThat(snapshot.state()).isEqualTo(RecursalLifecycleState.AGUARDANDO_PRECATORIO);
        assertThat(snapshot.autoridadeAtual()).isEqualTo(RecursalAuthority.SECRETARIA_JUDICIARIA);
        assertThat(stateMachine.availableEvents(snapshot, species, routePlan)).containsExactly(RecursalTransitionEvent.EXPEDIR_PRECATORIO);
    }

    @Test
    void devePermitirTransitoRastreavelERetornoParaAdmissibilidadeQuandoRemessaForDevolvida() {
        RecursalCaseContext context = upperCourtContext(false);
        RecursalSpecies species = new RecursoEspecial(true, true, false, false);
        RecursalRoutePlan routePlan = catalog.route(context, species);
        RecursalStateSnapshot snapshot = stateMachine.initialSnapshot("resp-remessa-devolvida", context, species, routePlan);
    ...
    private RecursalStateSnapshot flowToMerit(RecursalStateSnapshot snapshot, RecursalCaseContext context, RecursalSpecies species, RecursalRoutePlan routePlan) {
        snapshot = move(snapshot, context, species, routePlan, RecursalTransitionEvent.PROTOCOLAR);
        snapshot = move(snapshot, context, species, routePlan, RecursalTransitionEvent.VALIDAR_TEMPESTIVIDADE);
        snapshot = move(snapshot, context, species, routePlan, RecursalTransitionEvent.DISPENSAR_PREPARO);
        snapshot = move(snapshot, context, species, routePlan, RecursalTransitionEvent.INTIMAR_CONTRARRAZOES);
    ...
    private RecursalCaseContext upperCourtContext(boolean requisicaoPagamentoPublico) {
